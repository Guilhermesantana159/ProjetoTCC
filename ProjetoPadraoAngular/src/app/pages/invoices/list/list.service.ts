...
function sort(invoices: ListModel[], column: SortColumn, direction: string): ListModel[] {
  if (direction === '' || column === '') {
    return invoices;
  } else {
...
export class ListService {
  ...
  private _invoices$ = new BehaviorSubject<ListModel[]>([]);
  ...
  private _search(): Observable<SearchResult> {
    ...
    // 1. sort
    let invoices = sort(datas, sortColumn, sortDirection);
    
    // 2. filter
    invoices = invoices.filter(country => matches(country, searchTerm, this.pipe));

    // 4. Date Filter       
    if(date){
      invoices = invoices.filter(country => new Date(country.date) >= new Date(Object.values(date)[0]) && new Date(country.date) <= new Date(Object.values(date)[1]));
    }
    else{
      invoices = invoices;
    }
    ...
    // 5. Status Filter
    if(status){
      invoices = invoices.filter(country => country.status == status);
    }
    else{
      invoices = invoices;
    }
    ...
    invoices = invoices.slice(this._state.startIndex - 1, this._state.endIndex);
