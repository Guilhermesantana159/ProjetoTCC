...
function sort(countries: ApplicationModel[]): ApplicationModel[] {
  // if (direction === '') {
  return countries;
  // } else {
...
function matches(country: ApplicationModel, term: string, pipe: PipeTransform) {
  return country.name.toLowerCase().includes(term.toLowerCase())
    || country.designation.toLowerCase().includes(term.toLowerCase())
    || country.date.toLowerCase().includes(term.toLowerCase())
    || country.contacts.toLowerCase().includes(term.toLowerCase())
...
export class ApplicationService {
  ...
  private _countries$ = new BehaviorSubject<ApplicationModel[]>([]);
